...
import { Users, Calendar, ArrowRight, CheckSquare, Sparkles, Ticket, BarChart3, Megaphone, PlusCircle, UserPlus, Settings2, Shield, Eye, Compass, FileCheck, List, Archive } from 'lucide-react';
    ...
    <div className="space-y-6">
       <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
             ...
             <Link href={`/clubs/${myClubId}#announcements`} className="block">
                 <Card className="h-full flex flex-col transition-colors hover:bg-muted/50">
                    <CardHeader className="min-h-[60px]">
                    ...
                    {myClubs.length > 0 ? (
                        ...
                        {myClubs.map((club) => (
                            <Link href={`/clubs/${club.id}`} key={club.id} className="block group">
                                <div className="flex items-start gap-4 rounded-lg border p-3 h-full transition-colors group-hover:bg-muted/50">
                                    <Image
                                    src={club.imageUrl}
                                    alt={club.name}
                                    width={56}
                        ...
                        <div className="flex flex-col items-center justify-center text-center text-muted-foreground p-8">
                            <Compass className="size-12 mb-4" />
                            <p className="font-medium">You haven't joined any clubs yet.</p>
                            <p className="text-sm">Explore clubs to find your community.</p>
